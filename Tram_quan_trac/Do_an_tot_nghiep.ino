...
#include <SPI.h>
#include <Wire.h>
#include <Adafruit_GFX.h> // Thư viện đồ họa chung của Adafruit
#include <Adafruit_SH110X.h> // Thư viện cho màn hình 

#define i2c_Address 0x3C  //địa chỉ I2C của màn hình OLED
...
bool autoWarning = DISABLE; // biến bật tắt cảnh báo tự động
// --------------------- Cảm biến DHT11 ---------------------
#include "DHT.h"
#define DHT11_PIN         26
...
float tempValue = 30;
float humiValue   = 60;
SimpleKalmanFilter tempfilter(2, 2, 0.001); //sai so cb, uoc luong, nhieu
...
#define RAIN_ON   0
#define RAIN_OFF  1
int rainCountPulse = 0;                       //đếm số lần phễu lật
int  R_Funnel = 0;                           // lưu trạng thái(0/1)
int  V_AmountOfWater = 0 ;                  // thể tích
int  S_Funnel = 0 ;                        //lưu trạng thái phễu
float onePulseValue = 0;                  // giá trị mưa tính bằng mm
uint32_t timeMillisRain   = 0;           // thời điểm cuối ghi nhận lần đếm
uint32_t timeCalulateRain = 0;          // thời điểm thực hiện tính toán 

float rainValue = 0;                
float rainValueMax = 0;
//-------------------------------- Khai báo cảm biến gió ------------------------------------
#define windSensor        13     //kết nối
#define WIND_ON   0
#define WIND_OFF  1
int windCountPulse = 0;         // biến đếm số xung
uint32_t timeMillisWind   = 0; 
uint32_t timeCalulateWind = 0; 
float D_Anemometer = 0;             // lưu đường kính
#define PI  3.14
float C_Anemometer = 0;             // D * PI
float windValue = 0;                
float windValueMax = 0;
bool trigWindBlynk = 0;

bool trigRainBlynk = 0;
bool rainSensorTrig = 0;            // cảnh báo mưa
...
//----------------------- Gửi giá trị dữ liệu lên blynk--------

void myTimer() {
    Blynk.virtualWrite(V0, tempValue);  //gửi nhiệt độ lên chân ảo V0
